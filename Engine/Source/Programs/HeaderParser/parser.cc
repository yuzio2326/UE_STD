...
bool Parser::ParseStatement()
{
  ...
  // HBKim
  if (token.token == "GENERATED_BODY")
  {
      while (true) { if (GetChar() == '\n') { break; } }
      return true;
  }
  // HBKim
  if (token.token == "BOOST_DESCRIBE_STRUCT")
  {
      while (true) { if (GetChar() == '\n') { break; }}
      return true;
  }
  // HBKim
  else if (!ClassName.empty() && token.token == ClassName)
  {
      if (!ParseClassConstructor(token))
      {
          ParseClassObjectInitailizerConstructor(token);
      }
  }
...
bool Parser::SkipDeclaration(Token &token)
{
  ...
  // Check UTF-8 BOM
  if (token.token.size() == 3 && token.token[0] == '\xEF' && token.token[1] == '\xBB' && token.token[2] == '\xBF')
      return true;
...
bool Parser::ParseClassConstructor(Token& token)
{
    if (ClassName.empty())
    {
        return false;
    ...
    /*if (MatchSymbol("="))
    {
        std::string ErrorMsg = ClassName + " constructor = symbol not support!";
        Error(ErrorMsg.c_str());
        return false;
    }*/
    writer_.StartObject();
    writer_.String("type");
    writer_.String("default constructor");
    writer_.String("line");
    writer_.Uint((unsigned)token.startLine);
    WriteCurrentAccessControlType();

    writer_.String("name");
    writer_.String(ClassName.c_str());

    writer_.EndObject();
    return true;
}

bool Parser::ParseClassObjectInitailizerConstructor(Token& token)
{
    if (ClassName.empty())
    {
        return false;
    ...
    if (!MatchIdentifier("FObjectInitializer"))
    {
        return false;
    }
    if (!MatchSymbol("&"))
    ...
    writer_.StartObject();
    writer_.String("type");
    writer_.String("object initializer constructor");
    writer_.String("line");
    writer_.Uint((unsigned)token.startLine);
    WriteCurrentAccessControlType();
