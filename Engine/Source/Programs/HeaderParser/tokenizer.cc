...
bool Tokenizer::GetToken(Token& token, bool angleBracketsForStrings, bool seperateBraces)
{
    ...
    {
        // HBKIM
        bool bFindAPI_RemoveSpaceCheck = false;
    retry:
        // Read the rest of the alphanumeric characters
        do
        {
            if (bFindAPI_RemoveSpaceCheck)
            {
                bFindAPI_RemoveSpaceCheck = false;
                if (c != ' ')
                {
        ...
        // HBKim
        if (token.token.rfind("_API") != std::string::npos)
        {
            bFindAPI_RemoveSpaceCheck = true;
            token.token.clear();
            goto retry;
        }
