...
FRotator Quaternion::Rotator() const noexcept
{
    const float SingularityTest = z * x - w * y;
    const float YawY = 2.f * (w * z + x * y);
    const float YawX = (1.f - 2.f * (pow(y, 2.f) + pow(z, 2.f)));
    ...
    if (SingularityTest < -SINGULARITY_THRESHOLD)
    {
        ...
        Roll = FRotator::NormalizeAxis(-Yaw - (2.f * atan2(x, w) * RAD_TO_DEG));
    }
    ...
    else
    {
        ...
        Yaw = (atan2(YawY, YawX) * RAD_TO_DEG);
        Roll = (atan2(-2.f * (w * x + y * z), (1.f - 2.f * (pow(x, 2.f) + pow(y, 2.f)))) * RAD_TO_DEG);
    }
    ...
    //static const float RAD_TO_DEG = (180.f) / XM_PI;

    //// https://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
    //FQuaternion q(z, x, y, w);
    //FRotator RotatorFromQuat;

    //// roll (z-axis rotation)
    //double sinr_cosp = 2 * (q.w * q.x + q.y * q.z);
    //double cosr_cosp = 1 - 2 * (q.x * q.x + q.y * q.y);
    //RotatorFromQuat.Roll = (float)std::atan2(sinr_cosp, cosr_cosp) * RAD_TO_DEG;

    //// pitch (x-axis rotation)
    //double sinp = 2 * (q.w * q.y - q.z * q.x);
    //if (std::abs(sinp) >= 1)
    //    RotatorFromQuat.Pitch = (float)std::copysign(XM_PI / 2, sinp) * RAD_TO_DEG; // use 90 degrees if out of range
    //else
    //    RotatorFromQuat.Pitch = (float)std::asin(sinp) * RAD_TO_DEG;

    //// yaw (y-axis rotation)
    //double siny_cosp = 2 * (q.w * q.z + q.x * q.y);
    //double cosy_cosp = 1 - 2 * (q.y * q.y + q.z * q.z);
    //RotatorFromQuat.Yaw = (float)std::atan2(siny_cosp, cosy_cosp) * RAD_TO_DEG;
...
FMatrix FReversedZPerspectiveMatrix(float HalfFOVX, float HalfFOVY, float MultFOVX, float MultFOVY, float MinZ, float MaxZ)
{
    return FMatrix(
        FPlane(MultFOVX / tan(HalfFOVX), 0.f, 0.f, 0.f),
        FPlane(0.f, MultFOVY / tan(HalfFOVY), 0.f, 0.f),
        FPlane(0.f, 0.f, ((MinZ == MaxZ) ? 0.f : MinZ / (MinZ - MaxZ)), 1.f),
        FPlane(0.f, 0.f, ((MinZ == MaxZ) ? MinZ : -MaxZ * MinZ / (MinZ - MaxZ)), 0.0f)
    );
}
...
{
    return FMatrix(
        FPlane(MultFOVX / tan(HalfFOVX), 0.0f, 0.0f, 0.0f),
        FPlane(0.0f, MultFOVY / tan(HalfFOVY), 0.0f, 0.0f),
