...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        struct Matrix : public XMFLOAT4X4
        {
            ...
            // Constants
            CORE_API static const Matrix Identity;
        };
        ...
        struct Quaternion : public XMFLOAT4
        {
            ...
            FRotator CORE_API Rotator() const noexcept;

            // Constants
            CORE_API static const Quaternion Identity;
        };
        ...
        struct Color : public XMFLOAT4
        {
            ...
            CORE_API const static Color White;
            CORE_API const static Color Gray;
            CORE_API const static Color Black;
            CORE_API const static Color Transparent;
            CORE_API const static Color Red;
            CORE_API const static Color Green;
            CORE_API const static Color Blue;
            CORE_API const static Color Yellow;
        };
        // Binary operators
...
using FLinearColor = DirectX::SimpleMath::Color;
using FVector2D = DirectX::SimpleMath::Vector2;
using FVector3D = DirectX::SimpleMath::Vector3;
using FVector = DirectX::SimpleMath::Vector3;
using FVector4D = DirectX::SimpleMath::Vector4;
using FMatrix = DirectX::SimpleMath::Matrix;
using FQuat = DirectX::SimpleMath::Quaternion;
using FPlane = DirectX::SimpleMath::Plane;

static FORCEINLINE bool Vector3NearEqual(const FVector3D& A, const FVector3D& B, const FVector3D& Tolerance) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat3(&A);
    XMVECTOR v2 = XMLoadFloat3(&B);
    return XMVector3NearEqual(v1, v2, Tolerance);
}

static FORCEINLINE bool QuaternionNearEqual(const FQuat& A, const FQuat& B, const FQuat& Tolerance) noexcept
{
    using namespace DirectX;
    XMVECTOR q1 = XMLoadFloat4(&A);
    XMVECTOR q2 = XMLoadFloat4(&B);
    return XMVector4NearEqual(q1, q2, Tolerance);
}


// TReversedZPerspectiveMatrix<T>::TReversedZPerspectiveMatrix
FMatrix CORE_API FReversedZPerspectiveMatrix(float HalfFOVX, float HalfFOVY, float MultFOVX, float MultFOVY, float MinZ, float MaxZ);
