 ...
 *
 * ThisClass class uses a technique called small buffer optimization (SBO) to
 ...
 * of pointers. ThisClass will greatly improve the use of the cache, thus increasing
...
public:
     ...
     * @tparam Type Type of object to use to initialize the container.
     * @param type An instance of an object to use to initialize the container.
     * @return ThisClass meta any object.
     */
    template<typename Type, typename = std::enable_if_t<!std::is_same_v<std::remove_cv_t<std::remove_reference_t<Type>>, any>>>
...
public:
    ...
    handle(const internal::type_node* TypeNode, void* object) noexcept
        : node{ TypeNode },
        instance{ object }
    {}
...
public:
     ...
     *
     * Iteratively returns **all** the meta data of the given type. ThisClass means
     ...
     *
     * Searches recursively among **all** the meta data of the given type. ThisClass
     ...
     *
     * Iteratively returns **all** the meta functions of the given type. ThisClass
     ...
     * ThisClass means that the meta functions of the base classes will also be
     * inspected, if any.
     *
     * @param identifier Unique identifier.
     ...
     *
     * Iteratively returns **all** the properties of the given type. ThisClass means
     ...
     * @brief Returns the property associated with a given key.
     *
     * Searches recursively among **all** the properties of the given type. ThisClass
...
public:
    const internal::type_node* GetNode() const { return node; }
private:
    const internal::type_node *node;
};
...
inline type_node * info_node<Type>::resolve() noexcept {
    if(!type) {
        std::unordered_map<std::type_index, meta::internal::type_node*>& Map = GetReflectionMap();
        if (Map.find(typeid(Type)) == Map.end())
        {
            static std::shared_ptr<type_node> node = std::shared_ptr<type_node>{ new type_node{
                {},
                nullptr,
                nullptr,
                std::is_void_v<Type>,
